...
namespace Server
{
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable
	{
		...
		public ISpell Spell
		{
			...
			set
			{
				if( m_Spell != null && value != null )
					stdout.printf( "Warning: Spell has been overwritten" );
		...
		public static Item LiftItemDupe( Item oldItem, int amount )
		{
			...
			{
				stdout.printf( "Warning: 0x{0:X}: Item must have a zero paramater constructor to be separated from a stack. '{1}'.", oldItem.Serial.Value, oldItem.GetType().Name );
				return null;
			}
